...
CopepodDensity.bootstrap.means.SE <- var(CopepodDensity.bootstrap.means)^0.5/length(CopepodDensity.bootstrap.means)^0.5
...
append(FishDensity.bootstrap.medians, FishDensity.median) # add the median calculated by normal theory (total:1001)
...
append(CopepodDensity.bootstrap.medians, CopepodDensity.median) # add the median calculated by normal theory (total:1001)
...
CopepodDensity.bootstrap.medians.SE <- var(CopepodDensity.bootstrap.medians)^0.5/length(CopepodDensity.bootstrap.medians)^0.5
...
k.bt <- 10
x.bt  <- matrix(c(rep(1, times=n.bt)), nrow=n.bt, ncol=1, byrow = FALSE)
...
for (i in 1:10) {
  ...
  x.bt  <- cbind(x.bt, bootstrap.Copepod.i) # combine the new column into the x.bt
...
append(bootstrap.β0, β1[1]) # add the SE(β0) calculated by normal theory (total:1001)
append(bootstrap.β1, β1[2]) # add the SE(β1) calculated by normal theory (total:1001)

bootstrap.β0.SE <- var(bootstrap.β0)^0.5/length(bootstrap.β0)^0.5
bootstrap.β0.SE
bootstrap.β1.SE <- var(bootstrap.β1)^0.5/length(bootstrap.β1)^0.5
bootstrap.β1.SE
