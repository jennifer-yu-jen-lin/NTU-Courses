...
```{r}
#compute k-means NHC around 3 mediods (clusters)

##  Partitioning Around Medoids
### Partitioning (clustering) of the data into k clusters “around medoids”, a more robust version of K-means.
y.pam = pam(dist.METHOD,k=3) 
plot(y.pam)
## Clustering Large Applications
y.clara = clara(data.std,k=5) 
plot(y.clara) ## ???

plot(y.clara, ask=TRUE)

clusplot(data.std, km$cluster, color=TRUE, shade=TRUE, 
  	labels=2, lines=0)
prcomp(5,data.std)
xxx <- prcomp(data, scale=T)

plot(prcomp(data.std))

autoplot(prcomp(data.std), data = data.std, colour=y.pam$clustering, frame = TRUE, frame.colour = y.pam$clustering)
autoplot(prcomp(data.std), data = data.std, colour=y.pam$clustering) # work


data.std.2 <- cbind(data.std,y.pam$clustering)
autoplot(prcomp(data.std.2), data = data.std.2, colour=data.std.2[,44], frame = TRUE, frame.colour = data.std.2[,44])
...
rect.hclust(y.eucl.ward.D,k=3) #cut by #clusters ?
